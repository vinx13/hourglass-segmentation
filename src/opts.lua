...
local function parse(arg)
    ...
    cmd:option('-dataset',        'voc', 'Dataset choice: mpii | flic')
    ...
    cmd:option('-task',           'segmentation', 'Network task: pose | pose-int')
    ...
    cmd:text()
    cmd:option('-nEpochs',           1000, 'Total number of epochs to run')
    cmd:option('-trainIters',        100, 'Number of train iterations per epoch')
    cmd:option('-trainBatch',          8, 'Mini-batch size')
    cmd:option('-validIters',       30, 'Number of validation iterations per epoch')
